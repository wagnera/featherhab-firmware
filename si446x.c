...
void si446x_prepare(void)
{
    ...
    si446x_setfreq(SI446x_FREQUENCY, AFSK_DEVIATION_HZ);
...
void si446x_setfreq(uint32_t frequency, uint32_t deviation)
{
    ...
    //char set_frequency_property_command[] = {0x11, 0x40, 0x04, 0x00, 0x38, 0x0e, 0x0c, 0x49};
    ...
    uint32_t x = ((uint64_t)(1ul << 18) * outdiv * deviation) / SI446x_VCXO_FREQ;
    char set_frequency_separation[] = { 
      0x11, 0x20, 0x03, 0x0A,
      (uint8_t)(x >> 16),
      (uint8_t)(x >> 8),
      (uint8_t)(x >> 0)
